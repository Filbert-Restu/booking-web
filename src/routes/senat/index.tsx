...
function RouteComponent() {
  ...
  const handleApprove = async (id: number) => {
    try {
      await documentService.approveDocument(id, 'Disetujui oleh Senat');
  ...
  const handleOpenDoc = (payload: {
    ...
    if (payload.mode === 'preview') {
      navigate({
        to: '/preview-dokumen',
        search: { doc: payload.doc, id: payload.booking.id, return: '/senat' } as any
      });
    ...
    <>
      <div className='mb-6'>
        ...
        <p className='text-gray-600 mt-1'>
          Review dan persetujuan peminjaman ruang oleh Senat
        </p>
      </div>
