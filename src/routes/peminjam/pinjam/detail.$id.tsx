...
function DetailPengajuanPage() {
    ...
    const loadPdf = useCallback(async (type: DocType) => {
        if (!docId || isNaN(docId)) return;
        setPdfLoading(true);
        try {
            const response = await api.get(`/documents/${docId}/file/${type}/pdf`, {
                responseType: 'blob',
            });
            ...
            <div className="grid grid-cols-1 lg:grid-cols-8 gap-6 items-start">
                            ...
                            <div className="relative">
                                {steps
                                    ...
                                    .map((step, idx, arr) => {
                                        ...
                                        return (
                                            <div key={step.id ?? step.step_order} className="flex gap-4">
                                                {/* Icon + garis vertikal */}
                                                <div className="flex flex-col items-center">
                                                    ...
                                                    {!isLast && (
                                                        <div className={`w-0.5 flex-1 my-1 ${isCompleted ? 'bg-blue-300' : 'bg-gray-200'
                                                ...
                                                <div className={`pb-5 flex-1 ${isLast ? '' : ''}`}>
                                                    <div className={`text-sm font-semibold ${isCompleted ? 'text-blue-700' : isActive ? 'text-gray-900' : 'text-gray-500'
                                                        }`}>
                                                        {step.step_name}
                                                    </div>
                                                    ...
                                                    <div className={`text-xs mt-0.5 ${isCompleted ? 'text-blue-500' : isActive ? 'text-blue-500 font-medium' : 'text-gray-400'
                                                        }`}>
                                                        Status: {isCompleted ? 'Selesai' : isActive ? 'Pending' : 'Pending'}
                                                    </div>

                                                    {stepLog && (
                                                        <div className="text-xs text-gray-500 mt-0.5">
                                                            <span className="font-semibold">Received:</span>{' '}
                                                            {new Date(stepLog.created_at).toLocaleString('id-ID', {
                                                                day: '2-digit',
                                                                month: 'short',
                                                                year: 'numeric',
                                                                hour: '2-digit',
                ...
                <div className="lg:col-span-5 bg-white rounded-lg border border-gray-200 p-5 space-y-4">
                    <div className="flex items-center justify-between">
                        <h2 className="text-base font-semibold text-gray-800">Preview Dokumen</h2>
                        <Select
